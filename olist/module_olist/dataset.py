...
def load_data(orders_path: Path, items_path: Path, customers_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Carrega os datasets de pedidos, itens de pedidos e clientes a partir de arquivos csv.
    ...
    Returns:
        ...
        items: DataFrame contendo os dados de itens dos pedidos.
...
def create_dataset(orders, itens, customers):
    ...
    data = orders.merge(
        ...
        on="order_id", 
        how="left",
        validate="one_to_one"  # Garante que cada pedido tenha no máximo um registro de itens agregado.
        )
