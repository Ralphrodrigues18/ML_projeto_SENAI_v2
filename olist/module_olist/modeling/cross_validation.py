...
def cross_validate_models(X_train: pd.DataFrame, y_train: pd.Series,):
    ...
    best_model_name = max(cv_results, key=lambda name: (cv_results[name]["f1_oof"]),)
    ...
    best_f1 = best_results["f1_oof"]
    best_pr_auc = best_results["pr_auc"]

    logger.success(f"Melhor modelo: {best_model_name}")
